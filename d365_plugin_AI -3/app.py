...
@app.route("/api/plugin_files/<project>", methods=["GET"])
def api_list_plugin_files(project):
    from plugin_project import find_plugin_files
    files = find_plugin_files(project)
    return jsonify({"files": files})

@app.route("/api/plugin_file/<project>/<filename>", methods=["GET"])
def api_get_plugin_file(project, filename):
    import os
    from plugin_project import get_project_dir
    file_path = os.path.join(get_project_dir(project), filename)
...
@app.route("/api/plugin_file/<project>/<filename>", methods=["POST"])
def api_save_plugin_file(project, filename):
    import os
    from plugin_project import get_project_dir
    data = request.get_json()
